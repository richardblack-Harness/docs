...
* [Create repositories.](config-repos/create-repo.mdx)
* [Import repositories.](config-repos/import-repo.mdx)
...
* Create, review, and merge [pull requests](/docs/category/pull-requests):
   ...
   * Assign [code owners](config-repos/rules.mdx) and create [branch rules](config-repos/rules.mdx).
* Use Harness Code repositories in your [Harness pipelines](pipelines/codebase-from-harness-code.mdx), and set up [triggers](pipelines/code-triggers.mdx) to automatically run pipelines based on activity in your Harness Code repositories. You can also use Harness Code Repository with the [Harness Git Experience](/docs/platform/git-experience/git-experience-overview).
* [Configure webhooks](config-repos/webhooks.mdx) to integrate your repositories with third-party applications.
