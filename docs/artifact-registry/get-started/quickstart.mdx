...
keywords: ["artifact registry", "artifact registry quickstart", "artifact registry getting started", "configure registry", "configure upstream proxy", "connect upstream proxy", "use registry"]
---

import DocVideo from "@site/snippets/components/DocVideo";
...
<iframe 
  src="https://app.tango.us/app/embed/e3650c96-80e9-414d-aba1-64cb0d4db24d" 
...
<Note>Upstream proxies are commonly used to manage dependencies. During a build, if an open-source dependency is not already cached in the upstream proxy, it will be fetched from a public repository. This ensures access to necessary dependencies while optimizing retrieval speed and reducing redundant downloads.</Note>
...
2. Enter the **Upstream Proxy Key**. This is the identifier or name for the proxy within Harness and is chosen by you. 
   ...
   <Tip>This proxy key must start with a letter and can only contain lowercase alphanumerics, `_`, `.` and `-`</Tip>
...
<Tab title="Interactive guide">
<iframe 
  src="https://app.tango.us/app/embed/4294875b-384e-4558-839c-3f96c225a928" 
  title="Configuring Upstream in Harness Workspaces"
  width="100%"
  height="500"
  frameBorder="0"
  name="youtube_embed"
...
<Tab title="Interactive guide">
<iframe 
  src="https://app.tango.us/app/embed/a6085408-4187-4d5d-90cd-08e4a2ee193d" 
  title="Use an Artifact Registry"
  width="100%"
  height="500"
  frameBorder="0"
  name="youtube_embed"
...
<Note>When you pull an artifact, the system first checks the local registry. If the artifact is not found, it automatically queries the upstream proxy. The proxy then attempts to retrieve the artifact from the designated external source, if available. This process ensures seamless access to artifacts, even if they are not initially stored in the local registry.</Note>
